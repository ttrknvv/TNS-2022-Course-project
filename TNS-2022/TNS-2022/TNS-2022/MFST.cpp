...
namespace MFST
{
	...
	Mfst::Mfst(LEXTABLE plex, GRB::Greibach pgrebach)
	{
		grebach = pgrebach;
		lex = plex;
		...
		{
			lenta[k] = TS(*lex.lextable.table[k].lexemaх);
		};
		lenta_position = 0;
	...
	bool Mfst::savediadnosis(RC_STEP prc_step)
	{
		bool rc = false;
		short k = 0;
	...
	bool Mfst::start(Log::LOG& log)
	{
		...
		case LENTA_END: 
				MFST_TRACE4(log, "------>LENTA_END")
					* log.stream << "----------------------------------------------------------------" << std::endl;
				sprintf_s(buf, MFST_DIAGN_MAXSIZE, "%d: всего строк %d, синтаксический аализ выполнен без ошибок", 0, lex.lextable.table[lex.lextable.size - 1].sn);
				*log.stream << std::setw(4) << std::left << 0 << ": всего строк " << lex.lextable.table[lex.lextable.size - 1].sn << ", синтаксический анализ выполнен без ошибок" << std::endl;
				rc = true;
			break;
